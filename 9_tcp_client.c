...
#define MY_TCP_SERV_IP_STR "192.168.1.3"
#define MY_TCP_SERV_PORT_NUM 30000
...
void app_main(void)
{
    ...
    */
    ...
    i_sockaddr_in.sin_port = htons(MY_TCP_SERV_PORT_NUM);
    i_sockaddr_in.sin_addr.s_addr = inet_addr(MY_TCP_SERV_IP_STR);
    ...
    while (1)
    {
        ...
        int recv_len = lwip_recv(socket_obj, recv_str, sizeof(recv_str), 0); // 阻塞
        ...
        else if (recv_len > 0)
        {
            ...
            ESP_LOGI("tcp", "收到服务器的消息:%s,长度%d\n", recv_str, recv_len); // 接收字符流不含结尾\0
            if (0 == strcmp("shutdown", recv_str))
            {
                lwip_shutdown(socket_obj, SHUT_RD);
                lwip_close(socket_obj);
                return;
            }
        }
